...
public class Bank {
	...
	public void openAccount(String accountid) throws AccountExistsException {
		if (accountlist.containsKey(accountid)) {
			...
			// accountlist.get(accountid); <-- Thats not how you add element into Hashtable thats why test did not throw AccountExistsException cause, hashtable was always empty
			Account account = new Account(accountid, currency);

			// Adding the account to the hashtable using account ID as the key
	...
	public void deposit(String accountid, Money money) throws AccountDoesNotExistException {
		if (!accountlist.containsKey(accountid)) { // added negation cause if accountlist contains account id that means the account exists
